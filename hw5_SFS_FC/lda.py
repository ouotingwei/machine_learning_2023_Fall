...
class LDA():
    ...
    def fit(self, x, y, C=1):
        ...
        if x.shape[1] == 1:  # If there is only one feature
            ...
            self.b = -0.5 * w_T * (mean_class_1 + mean_class_2) - np.log(C * (p2 / p1))
            ...
            self.b = -0.5 * w_T @ (self.mean_class_1 + self.mean_class_2) - np.log(C * (p2 / p1))

            w_T = np.round(w_T, 2)
            self.b = round(self.b, 2)

        self.w_T = np.array([w_T])  # Ensure w_T is a vector
        self.b = 0 if x.shape[1] == 1 else self.b  # Set a default value for b if using multiple features
